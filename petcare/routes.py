...
from petcare import app, mongo, bcrypt, oauth
from .forms import (RegistrationForm, AddPet, LoginForm, AddVet, VetRegistrationForm, VetLoginForm,
                    EditProfileForm, CompleteVetRegistrationForm, CompleteUserRegistrationForm,
                    country_codes)  # Import the new form
from flask_login import login_user, logout_user, login_required, current_user
from .models import User, Pet, HealthReport, VetLicense
from bson.objectid import ObjectId
import pickle, numpy, pandas, secrets
from datetime import datetime
from flask import render_template, redirect, url_for, flash, request
# Import Mail and Message for sending emails
from petcare import app, mongo, bcrypt, mail
from flask_mail import Message
# Import the serializer for generating secure tokens
from itsdangerous import URLSafeTimedSerializer
from .forms import RegistrationForm # ... and other forms
from flask_login import login_user, logout_user, login_required, current_user
...
def verify_email(token):
    ...
    # ** THE FIX: Check if the user is a vet and redirect accordingly **
    if user.get('is_vet'):
        return redirect(url_for('vet_login_page'))
    else:
        return redirect(url_for('login_page'))
...
def login_page():
    ...
    if form.validate_on_submit():
        ...
        if user_data and not user_data.get('is_vet') and bcrypt.check_password_hash(user_data['password'],
                                                                                    form.password.data):
            user = User(user_data)
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        update_data = {
            # ** Email is no longer included in the update **
    ...
    form.email.data = current_user.email  # Email is still pre-populated
...
def add_pet_page():
    ...
    # ** UPDATED LOGIC **
    # The form's choices must be updated based on the submitted data for validation to work.
    if request.method == 'POST':
        species = request.form.get('pet_species')
        if species == 'Dog':
            form.pet_breed.choices = form.dog_breeds
        elif species == 'Cat':
            form.pet_breed.choices = form.cat_breeds
        else:  # Horse
            form.pet_breed.choices = form.horse_breeds
    else:
        # Set default choices for the initial GET request
        form.pet_breed.choices = form.dog_breeds
    ...
    if form.validate_on_submit():
        ...
        mongo.db.pets.insert_one({
            ...
            "breed": form.pet_breed.data,
            "gender": form.pet_gender.data,
            "dob": dob_datetime,
    ...
    # ** NEW: Pass all breed data to the template for JavaScript **
    breeds_data = {
        'Dog': form.dog_breeds,
        'Cat': form.cat_breeds,
        'Horse': form.horse_breeds
    }
...
def select_pet_page():
    ...
    return render_template('select_pet.html', pets=all_pets)
...
def analyze_pet_health_page(pet_id):
    ...
    # Pass calculated age and breed to the template
    return render_template(
        'analyze_pet_health.html',
...
def predict_health():
    """
    Analyzes pet health data, with robust handling for empty numeric inputs
    and server-side validation of vital signs.
    """
    try:
        data = request.get_json()

        # --- 1. Server-Side Validation of Vital Signs ---
        vital_ranges = {
        ...
        # --- THE FIX: Provide a default of '0' for empty numeric strings ---
        # This prevents the ValueError if a required field is empty.
        ...
        if species in vital_ranges:
            ...
            if not (ranges['temp'][0] <= temp <= ranges['temp'][1]):
                return jsonify({
                                   "error": f"Impossible Temperature for a {species}. Must be between {ranges['temp'][0]} and {ranges['temp'][1]}."}), 400
            if not (ranges['hr'][0] <= hr <= ranges['hr'][1]):
                return jsonify({
                                   "error": f"Impossible Heart Rate for a {species}. Must be between {ranges['hr'][0]} and {ranges['hr'][1]}."}), 400
            if not (ranges['rr'][0] <= rr <= ranges['rr'][1]):
                return jsonify({
                                   "error": f"Impossible Respiratory Rate for a {species}. Must be between {ranges['rr'][0]} and {ranges['rr'][1]}."}), 400

        # --- 2. Prepare Input DataFrame (if validation passes) ---
        ...
        # --- 4. Align with Model and Predict ---
        ...
        # --- 5. Generate and Return Result ---
        # (This part remains the same)
        result = {}
        if predicted_status == 'Healthy':
            result = {'status': 'Healthy', 'description': 'Overall health is excellent.', 'health_score': 100,
                      'confidence': f'{confidence}%', 'immediate_actions': ['Continue with regular care.'],
                      'lifestyle_changes': ['Maintain a balanced diet and regular exercise.'],
                      'monitoring_plan': ['Annual check-ups.'],
                      'vet_care': {'Annual dental cleaning': 'Medium', 'Regular parasite prevention': 'Medium'}}
        elif predicted_status == 'At Risk':
            result = {'status': 'At Risk', 'description': 'Minor issues detected that require monitoring.',
                      'health_score': 65, 'confidence': f'{confidence}%',
                      'immediate_actions': ['Schedule a non-urgent vet consultation.', 'Monitor symptoms closely.'],
                      'lifestyle_changes': ['Review diet for potential allergens.'],
                      'monitoring_plan': ['Check vital signs daily.'],
                      'vet_care': {'Diagnostic tests (blood work)': 'High', 'Follow-up consultation': 'High'}}
        else:  # Unhealthy
            result = {'status': 'Unhealthy', 'description': 'Significant health concerns detected.', 'health_score': 30,
                      'confidence': f'{confidence}%', 'immediate_actions': ['Seek veterinary care immediately.'],
                      'lifestyle_changes': ['Provide a comfortable resting area.'],
                      'monitoring_plan': ["Follow the veterinarian's treatment plan precisely."],
                      'vet_care': {'Emergency veterinary visit': 'Critical', 'Advanced imaging': 'High'}}
...
def delete_health_report(report_id):
    """Deletes a specific health report with strict ownership check."""
    ...
    # ** NEW, SIMPLIFIED LOGIC: Only the pet owner can delete. **
...
def delete_selected_reports():
    """Deletes multiple health reports with a strict ownership check."""
    ...
    # ** NEW, SIMPLIFIED LOGIC: Only the pet owner can perform this action. **
    ...
    result = mongo.db.health_reports.delete_many({
        "_id": {"$in": report_ids}
    })
    ...
    try:
        ...
        if existing_user:
            ...
            if intended_role == 'vet' and not user.is_vet:
                flash("This email is registered to a Pet Owner account. Please log in as a Pet Owner.", "danger")
                return redirect(url_for('vet_login_page'))
            login_user(user)
            ...
            new_user_data = {
                ...
                "phone": "", "address": "", "is_vet": False, "is_admin": False
            }
            ...
            if intended_role == 'vet':
                ...
                # ** NEW: Redirect normal users to their own completion page **
...
# ** NEW ROUTE: For completing pet owner registration **
...
def add_consulted_pet(pet_id):
    ...
    existing_link = mongo.db.vet_pet_links.find_one({
        "vet_id": ObjectId(current_user.id), "pet_id": ObjectId(pet_id)
    })

    if existing_link:
        flash("This pet is already in your consulted list.", "info")
    else:
        mongo.db.vet_pet_links.insert_one({
            "vet_id": ObjectId(current_user.id), "pet_id": ObjectId(pet_id), "date_added": datetime.utcnow()
        })
...
def remove_consulted_pet(pet_id):
    ...
    mongo.db.vet_pet_links.delete_one({
        "vet_id": ObjectId(current_user.id), "pet_id": ObjectId(pet_id)
    })
    flash("Pet removed from your consulted list.", "success")
    # Can redirect to either dashboard or select_pet page
